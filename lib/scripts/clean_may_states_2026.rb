...
class MayStatesCleaner
  INPUT_DIR  = Rails.root.join('data', '2026_states', 'may_states')
  OUTPUT_DIR = Rails.root.join('data', '2026_states', 'cleaned')

  # Map source filename stem → state abbreviation
  STATE_MAP = {
    'Iowa'              => 'IA',
    'Maine'             => 'ME',
    'NorthDakota'       => 'ND',
  ...
  # Files in this batch use "Yes"/"No"/blank for Incumbent. Blank → false.
  ...
  # Most files use "Withdrew" (blank or Yes/No). California uses "Withdrawn" — keep
  # the fallback for safety even though it's not in this batch.
  def parse_withdrew(row)
    val = row['Withdrew'] || row['Withdrawn']
    return 'false' if val.nil? || val.strip.empty?
    val.strip.downcase == 'yes' ? 'true' : 'false'
  end
  ...
  def standardize_party(party)
    ...
    when 'd', 'democrat', 'democratic', 'democract' then 'Democratic'  # NorthDakota has typo "Democract"
    ...
    when 'no party preference', 'npp'               then 'No Party Preference'
    when 'peace and freedom'                        then 'Peace and Freedom'
    when 'unaffiliated'                             then 'Unaffiliated'
    else party.strip
    end
  end
  ...
  def clean_url(url)
    ...
    # Some Facebook URLs in this batch have a trailing '#' — strip it.
    u = u.sub(/#\z/, '')
    u
  end
