...
class April7StatesCleaner
  INPUT_DIR = Rails.root.join('data', '2026_states', 'april7-states')
  ...
  def run
    puts "Cleaning 4 April 7 state CSVs..."
  ...
  def process_state(state_name, state_code)
    ...
    # Georgia has an extra blank column between CandidateName and Incumbent
    # Detect this by checking if header[2] is blank
    raw_headers = CSV.open(file, &:readline)
    has_blank_column = raw_headers[2].nil? || raw_headers[2].strip.empty?

    csv = CSV.read(file, headers: true, encoding: 'UTF-8')

    csv.each do |row|
      # Georgia: the blank column shifts Incumbent into a nil-keyed column
      # Re-map if needed
      if has_blank_column
        row = remap_georgia_row(row)
      end
  ...
  # Georgia CSV has headers: CandidateName, Incumbent, (blank), Party, ...
  # The blank column means the Incumbent value is actually in the header-less column
  # and Party etc. shift. We re-map to standard column names.
  def remap_georgia_row(row)
    # The CSV parser reads the blank header as nil or empty string
    # Row values by position: [0]=CandidateName, [1]=Incumbent, [2]=blank, [3]=Party, ...
    # But with the blank header, the actual Incumbent value lands under header "Incumbent"
    # and the blank column value is under nil/"" header. Let's just use the row as-is
    # since CSV.read with headers maps correctly based on header names.
    # The blank column is just an extra column that doesn't affect named access.
    row
  end
  ...
  # These files use "Yes"/"No" or blank for incumbent (not "X" like batch 2)
  def parse_incumbent(row)
    val = row['Incumbent']
    return 'false' if val.nil? || val.strip.empty?
    val.strip.downcase == 'yes' ? 'true' : 'false'
  end
  ...
  def standardize_party(party)
    ...
    when 'legal marijuana now'         then 'Legal Marijuana NOW'
    when 'non-partisan/independent', 'nonpartisan' then 'Nonpartisan'
    else party.strip
    end
  end
...
cleaner = April7StatesCleaner.new
cleaner.run
